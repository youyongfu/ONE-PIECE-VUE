<template>
    <div>
        ...
        <el-form :inline="true">
            <!-- 操作按钮-->
            <el-form-item>
                <el-button type="primary" @click="editHandle(null)" v-if="hasAuth('sys:episodes:save')">新增</el-button>
            </el-form-item>
        </el-form>
        ...
        <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" border stripe v-loading="loading">
            ...
            <el-table-column prop="icon" label="操作">
                <template slot-scope="scope">
                    ...
                    <el-button type="text" slot="reference" @click="deleteHandle(scope.row.id)" v-if="hasAuth('sys:episodes:delete')">删除</el-button>
                </template>
            </el-table-column>
        ...
        <EpisodesEdit title="编辑剧集信息" v-if="dialogVisible" ref="EpisodesEdit"/>
    </div>
</template>

<script>
    import EpisodesEdit from "../data/EpisodesEdit";

    export default {
        name: "Episodes",
        components:{EpisodesEdit},
        data() {
            return {
        ...
        methods: {
            //获取
            listPage() {
                this.$axios.get('sys/episodes/listPage', {params: {
            ...
            editHandle(id){
                ...
                this.$nextTick(() => {
                    this.$refs.EpisodesEdit.init(id);
                });
            },
            //删除
            deleteHandle(id){
                this.$confirm('此操作将删除该剧集, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.$axios.post("/sys/episodes/delete/" + id).then(res => {
                        console.log(res)
                        this.$message({
                            showClose: true,
                            message: '删除成功',
